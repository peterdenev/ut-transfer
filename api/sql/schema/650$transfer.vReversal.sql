...
JOIN
    [transfer].[partner] p ON (((
        (t.issuerTxState = 2 AND ISNULL(t.acquirerTxState, 0) IN (0, 1, 3, 4, 5) AND p.mode = 'online' AND t.channelType IN ('ATM')) OR --tx succeeded at issuer during online but failed at acquirer and current mode is online
        (t.issuerTxState = 8 AND ISNULL(t.acquirerTxState, 0) IN (0, 1, 3, 4, 5) AND p.mode IN ('online', 'offline')) OR --tx succeeded at issuer during offline but failed at acquirer and current mode is online/offline
